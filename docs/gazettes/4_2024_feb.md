...
The tagline of `xan` will be a dorky `Xan, the CSV magician` and will be described as "The most overengineered piece of code ever written to process CSV data".
...
The selection DSL has been updated to accept 0-based indices instead of 1-based ones, to remain consistent across the board.
...
This also means the `headers` command will now index columns in a 0-based fashion.

### No more trimming

This was not entirely documented but `xsv` used to trim cell values when using the `join` or `frequency` command. This can be useful sometimes but 1. it has a slight performance cost, 2. it can be devious when dealing with messy data and you want to investigate the clerical mistakes.
...
I took great length to make sure the `groupby` command can be as fast as possible:
...
Then, asking for instance for both the variance and the mean or the mode and the cardinality will never be more costly since their requirements in terms of memory and computation are the same.
...
Before, it was not possible for a `moonblade` expression to return a literal or identifier.

This is now a thing of the past.

```js
// This was examples of invalid expressions:
col_name
true
45
67.8
...
You can now do it using the `--input` flag.
...
But things are more complex if you want to match a large bunch of regexes. So to be sure we don't have to linearly test every given regex pattern, we rely on [RegexSet](https://docs.rs/regex/latest/regex/struct.RegexSet.html) black magic to be as fast as possible.

*Similarity to join*

In a sense `search --input` a specialization of an inner `join` command. But in this case, you are not interested in any column attached to your query file and you want to make sure the query will be indexed and not the other way around.
...
The expression evaluated by the `transform` command starts with an implicit value of target column:
...
* `moonblade` function arity is now checked when "compiling" the expression before it can be run. This makes things more efficient across the board. This also mean users will be notified they messed up function arities before even needing to evaluate the expression.
