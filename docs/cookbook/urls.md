...
- [Use case](#use-case)
- [Basic `xan join -u/--url-prefix` usage](#basic-xan-join--u--url-prefix-usage)
...
In this guide I will show you how to use the `xan join -u/--url-prefix` command to "join" (i.e. match rows) two CSV files containing urls.
...
## Basic `xan join -u/--url-prefix` usage
...
```bash
xan join -u url tweets.csv homepage medias.csv > joined.csv
```
...
xan join -u url tweets.csv homepage medias.csv -R media_ | xan select :2,media_politics > joined.csv
```
...
To handle this, `xan` can be told to "explode" the file before passing it to `join` downstream so you can represent your entities thusly:
...
You can then pipe `xan explode` into `xan join` to consider each prefix as a distinct row:

```bash
# Note how "-" is meant to represent stdin in the join command
xan explode prefixes --sep " " --rename prefix medias.csv | \
xan join -u prefix - url tweets.csv > joined.csv
```
...
```bash
xan join -u url tweets.csv homepage medias.csv --left | xan select tweet_id,media > joined.csv
```
