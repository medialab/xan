...
pub fn get_function(name: &str) -> Option<(Function, Arity)> {
    Some(match name {
        ...
        "bytesize" => (bytesize, Arity::unary()),
        "ceil" => (
            |args| unary_arithmetic_op(args, DynamicNumber::ceil),
            Arity::unary(),
        ),
        "coalesce" => (coalesce, Arity::Min(1)),
        "compact" => (compact, Arity::unary()),
        "concat" => (concat, Arity::Min(1)),
        "contains" => (contains, Arity::binary()),
        "copy" => (copy_file, Arity::binary()),
        "count" => (count, Arity::binary()),
        "div" => (|args| variadic_arithmetic_op(args, Div::div), Arity::Min(2)),
        "endswith" => (endswith, Arity::binary()),
        "err" => (err, Arity::unary()),
        "escape_regex" => (escape_regex, Arity::unary()),
        "ext" => (ext, Arity::unary()),
        "filesize" => (filesize, Arity::unary()),
        "first" => (first, Arity::unary()),
        "floor" => (
        ...
        "isfile" => (isfile, Arity::unary()),
        "join" => (join, Arity::binary()),
        "json_parse" => (json_parse, Arity::unary()),
        "keys" => (keys, Arity::unary()),
        "last" => (last, Arity::unary()),
        "len" => (len, Arity::unary()),
        "log" => (
        ...
        "neg" => (|args| unary_arithmetic_op(args, Neg::neg), Arity::unary()),
        "not" => (not, Arity::unary()),
        "or" => (or, Arity::Min(2)),
        ...
        "startswith" => (startswith, Arity::binary()),
        ...
        "typeof" => (type_of, Arity::unary()),
        "unidecode" => (apply_unidecode, Arity::unary()),
        "upper" => (upper, Arity::unary()),
        "uuid" => (uuid, Arity::Strict(0)),
        "values" => (values, Arity::unary()),
        "write" => (write, Arity::binary()),
        _ => return None,
    })
}
