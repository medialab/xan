...
Deleting all files whose filenames are listed in a column:
  ...
  $ xan foreach filename 'rm {}' assets.csv

Executing a command that outputs CSV once per line without repeating headers:

  $ xan foreach query 'search --year 2020 {}' queries.csv > results.csv

Same as above but with an additional column containing the current value:

  $ xan foreach query -c from_query 'search {}' queries.csv > results.csv

Usage:
    xan foreach [options] <column> <command> [<input>]
    xan foreach --help
