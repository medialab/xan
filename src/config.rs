...
impl TabularDataKind {
    fn has_headers(&self) -> bool {
        !matches!(
            self,
            Self::Ndjson | Self::Gtf | Self::Sam | Self::Bed | Self::Mtx
        )
    }
    ...
    fn header_pattern(&self) -> Option<Regex> {
        match self {
            ...
            Self::Mtx => Some(Regex::new("^%").unwrap()),
            _ => None,
        }
    }
}
...
impl Config {
        ...
        let (path, delimiter, compression, tabular_data_kind) = match *path {
            ...
            Some(ref s) => {
                ...
                let delim = if raw_s.ends_with(".tsv") || raw_s.ends_with(".tab") {
                    ...
                    b'\t'
                } else if raw_s.ends_with(".mtx") {
                    kind = TabularDataKind::Mtx;
                    b' '
                } else {
                    b','
                };
    ...
    pub fn simd_peeker(&self) -> CliResult<simd_csv::Peeker<Box<dyn io::Read + Send + 'static>>> {
        Ok(self.simd_csv_peeker_from_reader(self.io_reader()?))
    }
    ...
    pub fn simd_csv_peeker_from_reader<R: Read>(&self, rdr: R) -> simd_csv::Peeker<R> {
        simd_csv::PeekerBuilder::new()
            .delimiter(self.delimiter)
            .quote(self.quote)
            .has_headers(!self.no_headers)
            .from_reader(rdr)
